...
if __name__ == '__main__':
    ...
    # Calcular el ancho total para las ventanas y las líneas divisorias
    ancho_total = ANCHO * NUM_CAPAS + NUM_CAPAS - 1 # Calcula el ancho total de la ventana sumando el ancho de todas las capas, agregando el número de líneas divisorias entre capas ###
    root.geometry(str(ancho_total) + "x" + str(ALTO)) # Configura la geometría de la ventana principal agregando la division, representa las dimensiones de la ventana y se establece como la geometría de la ventana ###
    ...
    for i in range(NUM_CAPAS):  # Se utiliza un bucle para crear un lienzo para cada capa
        ...
        # Dibujar líneas divisorias entre las capas, excepto después de la última capa
        if i < NUM_CAPAS - 1:# La condición asegura que las líneas divisorias solo se dibujen entre capas y no después de la última capa ###
            lienzo.create_line((i + 1) * ANCHO, 0, (i + 1) * ANCHO, ALTO, fill="black")#"create_line" es un método de la clase Canvas de Tkinter, que se utiliza para crear líneas en un lienzo###
        ...
        for i, lienzo in enumerate(lienzos):  # Itera sobre cada lienzo en la lista de lienzos y su índice
            ...
            for y in range(ALTO_CAPA):  # Itera sobre las filas de la capa actual
                for x in range(ANCHO_CAPA):  # Itera sobre las columnas de la capa actual
                    ...
                    # Dibujar líneas adicionales para dividir en 9 cuadrados###
                    if x % (ANCHO_CAPA // 3) == 0 and x > 0:#Se Verifica si estamos en el borde derecho de uno de los cuadrados más pequeños y evitando líneas en el borde izquierdo del lienzo ###
                        #Dibujar las líneas de forma vertical ###
                        lienzo.create_line(x * TAMANO_CELDA, y * TAMANO_CELDA, x * TAMANO_CELDA, (y + 1) * TAMANO_CELDA, fill="white", width=2, tags="capa")
                    if y % (ALTO_CAPA // 3) == 0 and y > 0:#Se verifica si estamos en el borde inferior de uno de los cuadrados más pequeños,, y se evitan las líneas en el borde superior del lienzo ###
                        #Dibujar las líneas de forma horizontal ###
                        lienzo.create_line(x * TAMANO_CELDA, y * TAMANO_CELDA, (x + 1) * TAMANO_CELDA, y * TAMANO_CELDA, fill="white", width=2, tags="capa")
            ...
            for y in range(ALTO_CAPA):  # Bucle anidado para iterar sobre cada célula de la capa actual
                for x in range(ANCHO_CAPA):
                    if capas[i][y][x] == SUSCEPTIBLE:# Se verifica si la célula actual en la capa es "SUSCEPTIBLE"
                        # Contar vecinos en la misma capa que están infectados y en el mismo cuadrado
                        vecinos_i_misma_capa = sum(1 for j in [-1, 0, 1] for k in [-1, 0, 1]# Bucles anidados para abarcar las posiciones relativas alrededor de la célula actual
                                                    ...
                                                    and x // (ANCHO_CAPA // 3) == (x + j) // (ANCHO_CAPA // 3) 
                                                    and y // (ALTO_CAPA // 3) == (y + k) // (ALTO_CAPA // 3))

                        # Contar vecinos en capas adyacentes que están infectados y en el mismo cuadrado
                        ...
                        # Aplicar reglas del modelo SEIR
                        if vecinos_i_misma_capa >= 1 or vecinos_i_capas_adyacentes >= 1: # Se verifica si hay al menos un vecino infectado en la misma capa o en capas adyacentes
                            nueva_capa[y][x] = EXPUESTO # La célula actual se marca como "EXPUESTO"
                        ...
                        if tiempo_expuesto[y][x] >= 1:  # Verifica que haya pasado al menos un tiempo para pasar a la siguiente fase
